#!/usr/bin/env Rscript
# Create a gene coexpression matrix for a baseline Expression Atlas
# experiment Input: undecorated tsv file, first column gene
# identifier, subsequent columns
...
if (length(args) == 5) {
    ...
    # Destination (if not finishing with .gz, clusterSeq will append
    # .gz to the end)
    outputPath <- args[2]
...
# Make sure there are at least three columns (gene ID, expression
# columns) -- doesn't make sense to try coexpression with only one or
# two columns of expression data.
...
exp[, 2:ncol(exp)] <- sapply(exp[2:ncol(exp)], function(x) sub("(^[^,]+[,][^,]+[,])([^,]+)(,.+$)","\\2", x))  #get the middle value for each gene/tissue
...
# run kCluster function to create coexpression matrices, set to use
# 16 cores.  It can be changed to use less or more

max_avail_workers <- multicoreWorkers()

use_cores <- max(1, min(cores, max_avail_workers - 2))

# retrial mechanism up to 5 retries in case of error/warning

for (i in 1:retries) {

    print(paste0("kClust will run with ", use_cores, " cores... Starting try number: ",
        i))
    out <- tryCatch({
