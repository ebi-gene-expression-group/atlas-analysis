...
version_info <- data.frame(
    V1 = "Deconvolution",
    V2 = paste0('Cell type proportion predictions of FPKMs per organism part with organism part specific scRNA-seq as reference. For reference creation see <a href=https://github.com/ebi-gene-expression-group/atlas-deconvolution-references>atlas-deconvolution-references</a>. Cell type predictions displayed are averages of results from EpiDISH (version: ', as.character(epidish_version),'), FARDEEP (version: ',  as.character(fardeep_version), ') and DWLS (version: ',  as.character(dwls_version), ')')
)
...
write.table(analysis_methods, file = paste0("/nfs/production/irene/ma/users/nnolte/DECONV_ATLAS/methods/", accession, '-analysis-methods.updated.tsv') , sep = "\t", row.names = FALSE, col.names = FALSE)
