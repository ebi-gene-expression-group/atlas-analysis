...
# diffAtlas_DE_limma.R Microarray differential expression statistics computation for Expression Atlas.
...
# diffAtlas_DE_limma() - Differential expression analysis (2-group comparison) using limma.  Arguments: expAcc <- ArrayExpress accession of
# experiment.  atlasProcessingDirectory <- path to Atlas processing directory.
diffAtlas_DE_limma <- function(expAcc, atlasProcessingDirectory) {
    ...
    e <- try({
        ...
        if (!grepl("array", experimentType)) {
            stop(paste("Experiment type", experimentType, "does not look like a microarray experiment. Cannot continue"))
        ...
        if (grepl("1colour", experimentType)) {
            ...
            cat(paste("Experiment type", experimentType, "not recognised. Cannot continue."))
        }
...
# run_one_colour_analysis - For a one-colour experiment, given an experiment accession, the list of analytics objects, and the Atlas processing
# directory, run the differential expression analysis defined in the contrasts contained in the analytics objects.
run_one_colour_analysis <- function(expAcc, allAnalytics, atlasProcessingDirectory) {
    ...
    invisible(sapply(allAnalytics, function(analytics) {
        ...
        # Read experiment, contrasts and expression. Subset expression to match the derived experiment.
        ...
        sapply(split(contrastsTable, contrastsTable$formula), function(fc) {
            ...
            normalizedDataForFormula <- normalizedData[, colnames(normalizedData) %in% experiment$BioRepName[experiment$assay_group_id %in%
                c(fc$reference_assay_group_id, fc$test_assay_group_id)]]
            ...
            contrastNames <- paste(paste("assay_group_id", make.names(fc$test_assay_group_id), sep = "."), paste("assay_group_id",
                make.names(fc$reference_assay_group_id), sep = "."), sep = "-")
...
# run_two_colour_analysis - For a two-colour experiment, given an experiment accession, the list of analytics objects, and the Atlas processing
# directory, run the differential expression analysis defined in the contrasts contained in the analytics objects. For each contrast only the samples
# pertinent to that contrast are modelled.
run_two_colour_analysis <- function(expAcc, allAnalytics, atlasProcessingDirectory) {
    ...
    invisible(sapply(allAnalytics, function(analytics) {
        ...
        apply(contrastsTable, 1, function(cont) {
            ...
            if (any(!sort(experiment$AssayNameNoCy[experiment$assay_group_id == cont["reference_assay_group_id"]]) == sort(experiment$AssayNameNoCy[experiment$assay_group_id ==
                cont["test_assay_group_id"]]))) {
            ...
            maList <- list(genes = rownames(logFCsForContrast), M = logFCsForContrast, A = avgIntsForContrast)
...
# Run with arguments if there are any, otherwise don't do anything. Having this lets us source this file in an R session and load all the functions
# so we can run them separately if desired.
