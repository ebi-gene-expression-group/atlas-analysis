...
my ($atlasExperimentDir, $idfFilename, $loadDir, $miRBaseDirectory, $abs_path, $expDir) = @ARGV;
...
my $normalizationRscript = "$abs_path/atlas-analysis/norm/arrayNormalization.R";
...
#my $atlasXMLconfigPath = File::Spec->catfile( $abs_path,"/", $atlasExperimentDir, $atlasXMLconfigFile );
my $atlasXMLconfigPath = File::Spec->catfile( $expDir,"/", $atlasXMLconfigFile ); 
